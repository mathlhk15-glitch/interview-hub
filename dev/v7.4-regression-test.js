...
ok('version is v7.4', /7\.4-rag-lite/.test(config));
ok('cache bust is v7.4', /style\.css\?v=7\.4/.test(index) && /screens2\.js\?v=7\.4/.test(index));
...
ok('print sheet exposes full-question print directly', /open-all-questions-print-btn/.test(screens2) && /예상질문 전체 인쇄/.test(screens2));
ok('print all questions button exists', /print-questions-btn/.test(screens2) && /openQuestionsPrintView/.test(screens2));
ok('global topbar print shortcut exists', /topbar-print-btn/.test(index) && /openQuestionsPrintView\(AppState\)/.test(index));
ok('screen-level print shortcut exists', /buildQuestionPrintShortcut/.test(screens) && /screen-quick-actions/.test(screens));
ok('dashboard print banner exists', /question-print-banner/.test(screens));
ok('trainer has print shortcut', /자가진단 체크하기/.test(screens2) && /🖨️ 질문지 전체 인쇄/.test(screens2));
ok('print-sheet has explicit all-question print button', /open-all-questions-print-btn/.test(screens2));
ok('flow nav has print shortcut', /flow-print-btn/.test(app) && /openQuestionsPrintView\(AppState\)/.test(app));
...
console.log(`v7.4 self trainer test finished. fail=${fail}`);
