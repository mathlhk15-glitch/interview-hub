...
ok('version is v7.5', /7\.5-rag-lite/.test(config));
ok('cache bust is v7.5', /style\.css\?v=7\.5/.test(index) && /screens2\.js\?v=7\.5/.test(index));
...
ok('print sheet exposes full-question print directly', /open-all-questions-print-btn/.test(screens2) && /모든 질문 인쇄/.test(screens2) && /open-saved-questions-print-btn/.test(screens2));
ok('dual print buttons exist', /print-all-questions-btn/.test(screens2) && /print-saved-questions-btn/.test(screens2) && /openAllQuestionsPrintView/.test(screens2) && /openSavedQuestionsPrintView/.test(screens2));
ok('global topbar has dual print shortcuts', /openAllQuestionsPrintView\(AppState\)/.test(index) && /openSavedQuestionsPrintView\(AppState\)/.test(index));
ok('screen-level dual print shortcuts exist', /buildQuestionPrintShortcuts/.test(screens) && /openAllQuestionsPrintView/.test(screens) && /openSavedQuestionsPrintView/.test(screens));
ok('dashboard print banner exists', /question-print-banner/.test(screens));
ok('trainer has dual print shortcuts', /자가진단 체크하기/.test(screens2) && /모든 질문 인쇄/.test(screens2) && /저장된 질문만 인쇄/.test(screens2));
ok('print-sheet has explicit all-question print button', /open-all-questions-print-btn/.test(screens2));
ok('flow nav has dual print shortcuts', /flow-print-all-btn/.test(app) && /flow-print-saved-btn/.test(app) && /openAllQuestionsPrintView\(AppState\)/.test(app));
...
ok('print modes have independent empty warnings', /인쇄할 질문이 없습니다/.test(printjs) && /저장된 질문이 없습니다/.test(printjs));
ok('all-print collector reads unsaved AI questions', /collectAllPrintableQuestions/.test(printjs) && /activityInventory/.test(printjs) && /priorityA/.test(printjs));
ok('saved-print uses question bank only', /savedPrintableQuestions/.test(printjs) && /state\.questions/.test(printjs));
...
// v7.5: 모든 질문 인쇄는 저장하지 않은 AI 질문까지 포함하고, 저장 질문 인쇄는 질문은행만 사용해야 함.
const dualState = {
  questions: [{ id:'saved1', text:'저장된 질문?', priority:'B', directionLabel:'저장' }],
  aiResultSections: {
    priorityA: [{ question:'저장하지 않은 A 질문?', depth:'concept' }],
    priorityB: [], priorityC: [],
    activityInventory: [{ title:'활동1', area:'세특', evidenceQuote:'근거', questions:[{ question:'활동별 질문?', type:'과정' }], followUpQuestions:['활동 꼬리질문?'] }],
    followUpQuestions: []
  }
};
const allQs = printCtx.collectAllPrintableQuestions(dualState);
const savedQs = printCtx.savedPrintableQuestions(dualState);
ok('all print includes unsaved AI questions', allQs.some((q)=>q.text==='저장하지 않은 A 질문?') && allQs.some((q)=>q.text==='활동별 질문?'));
ok('saved print excludes unsaved AI questions', savedQs.length===1 && savedQs[0].text==='저장된 질문?');
vm.runInContext(`AppState.universities = [{ id: 'u1', name: '테스트대학교', major: '테스트학과', track: '테스트전형' }]; AppState.activeUniversityId = 'u1';`, printCtx);
...
console.log(`v7.5 self trainer test finished. fail=${fail}`);
